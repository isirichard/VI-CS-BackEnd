...
//	Agregar al Inventario el producto
	...
	public boolean IncrementarProducto(Inventario i) {
...
//		Aquí se hace la agregación del stok en el inventario en el caso de haber un stok anteriormente 
	...
	public boolean validarProducto(Producto p) {
		int resultado=0;
		String sql1 = "select Producto.ProdCod, Producto.ProSKU, Producto.ProdDes, Producto.TipProCod, Producto.MarCod, Producto.UniMedCod"
				+ "From Producto"
				+ " where (Producto.ProSku = '"+p.getProSKU()+"')" ;
		...
		try {
			...
			if(rs1.next()) {
				...
				if(resultado == 1) {
					...
					p.setProdCod(rs1.getInt("Producto.ProdCod"));
					Tipo_Producto tipo = new Tipo_Producto();
					tipo.setCodigo(rs1.getInt("Producto.TipProCod"));
					p.setTipProCod(tipo);
					Marca marca=new Marca();
					marca.setDescripcion(rs1.getString("Marca.MarDes"));
					marca.setCodigo(rs1.getInt("Producto.MarCod"));
					Unidad_Medida undMed = new Unidad_Medida();
					undMed.setDescripcion(rs1.getString("Unidad_Medida.UniMedDes"));
					undMed.setCodigo(rs1.getInt("Producto.UniMedCod"));
					p.setProSKU(rs1.getInt("Producto.ProSKU"));
					p.setProdDes(rs1.getString("Producto.ProdDes"));
					p.setMarCod(marca);
					p.setUniMedCod(undMed);
